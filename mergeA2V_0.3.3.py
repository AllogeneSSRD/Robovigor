...
class CustomProgressLogger:
    ...
    def __call__(self, message):
        """可调用接口现在会打印原始消息"""
        # 打印到控制台（调试用）
        print("[RAW MESSAGE]", repr(message))  # 原始消息的转义字符
    ...
    def bars_callback(self, bar, attr, value, old_value=None):
        ...
        if attr == 'total':
            ...
            line = f"{percent:.1f}%|{elapsed}<{remaining}, {self.bar.format_dict['rate']:.2f}it/s"
...
class VideoProcessor:
    ...
    def mp4_to_mp4(self):
        ...
        ffmpeg_write_video(
            ...
            bitrate=None,
            write_logfile=False,
            logger=self.logger  # 传递自定义logger
        )
        if self.logger:
            self.logger.close()

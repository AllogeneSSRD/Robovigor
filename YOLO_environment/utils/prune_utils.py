...
def weights_inheritance(model, compact_model, from_to_map, maskbndict):
    ...
    for ((layername, layer), (pruned_layername, pruned_layer)) in zip(model.named_modules(),
                                                                      compact_model.named_modules()):
        assert layername == pruned_layername
        if isinstance(layer, nn.Conv2d) and not layername.startswith(
                f"model.{last_idx}"):  # --------------------------------
            ...
            if convname in from_to_map.keys():
                ...
                if isinstance(former, list):
                    ...
                    former_kernel_num = [modelstate[former_item + ".weight"].shape[0] for former_item in
                                         former]  # Compute former_kernel_num for accumlate index
                    in_idx = []
        ...
        if isinstance(layer, nn.Conv2d) and layername.startswith(
                f"model.{last_idx}"):  # --------------------------------

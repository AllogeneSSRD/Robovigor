...
class Model(nn.Module):
    def __init__(self, cfg='yolov5s.yaml', ch=3, nc=None, anchors=None):  # model, input channels, number of classes
        ...
        if isinstance(m, Detect) or isinstance(m, IKeypoint):
            s = 256  # 2x min stride
...
def parse_model(d, ch):  # model_dict, input_channels(3)
    ...
    for i, (f, n, m, args) in enumerate(d['backbone'] + d['head']):  # from, number, module, args
        ...
        if m in [Conv, GhostConv, Bottleneck, GhostBottleneck, SPP, DWConv, MixConv2d, Focus, CrossConv, BottleneckCSP,
                 C3, C3TR, SPPCSPC, SPPFCSPC, C2f, CBRM, Shuffle_Block,SPPF]:
            c1, c2 = ch[f], args[0]
            if c2 != no:  # if not output
                c2 = make_divisible(c2 * gw, 8)
                # ------ Pruning parts --------
                if isinstance(args[-1], float) and m not in [SPP, SPPF, SPPCSPC,SPPFCSPC]:
                    c2 = c2 * args[-1]
                    args = args[:-1]
            ...
            args = [c1, c2, *args[1:]]
            if m in [BottleneckCSP, C3TR, SPPCSPC, SPPFCSPC, C2f]:
